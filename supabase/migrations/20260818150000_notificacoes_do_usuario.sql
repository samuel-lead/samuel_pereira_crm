-- Notificações do sino no menu lateral. Não existe tabela de notificação —
-- são calculadas na hora, direto do estado atual (reunião de hoje, reunião
-- cancelada recente, lead parado). RLS das tabelas de baixo já restringe
-- por org; aqui só filtramos pelo usuário logado (dono do lead/reunião).
