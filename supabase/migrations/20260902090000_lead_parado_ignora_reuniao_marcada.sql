-- "Lead parado" olhava só a última interação e quando entrou no nível,
-- ignorando se já tinha reunião marcada — um lead com reunião marcada
-- pra 3 semanas na frente aparecia como "parado" só porque ninguém
-- registrou uma interação nova nesse meio tempo. Reunião marcada É o
-- próximo passo combinado, não é lead parado. Aplica em SQL puro (regra
-- CLAUDE.md 5) — tanto no sininho quanto no aviso automático de push.
...
-- Mesma correção na função que dispara o push automático.
