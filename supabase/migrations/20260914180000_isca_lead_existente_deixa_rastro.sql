-- Samuel pediu: quando quem já é lead responde outra isca (ou a mesma de
-- novo) com o mesmo WhatsApp, isso não pode ficar invisível pra ele. Antes,
-- "já existia" só retornava sucesso pra pessoa e não gravava nada em lugar
-- nenhum — o Samuel nunca ficava sabendo que ela voltou a interagir.
-- Continua sendo 1 pessoa = 1 card no Kanban (não duplica lead, não infla
-- a métrica de leads trabalhados), mas agora grava a resposta da isca nova
-- e uma nota na linha do tempo do lead já existente — aparece pra ele e
-- o lead volta a contar como "ativo recentemente".
...
begin
  ...
  if v_lead_id is not null then
    ...
    insert into interacoes (org_id, usuario_id, lead_id, tipo, canal, conteudo, ocorreu_em, origem)
    values (
      v_isca.org_id,
      v_isca.usuario_id,
      v_lead_id,
      'nota',
      'manual',
      'Respondeu de novo pela isca "' || v_isca.nome || '".',
      now(),
      'declarado'
    );
  else
