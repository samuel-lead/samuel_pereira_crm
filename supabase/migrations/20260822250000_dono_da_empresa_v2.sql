-- "Dono da empresa": pra cada empresa cliente, a pessoa que a criou (o
-- admin que o Samuel cadastra ao dar de alta um cliente novo) já nasce
-- marcada como dona daquela empresa — sem precisar de passo manual. Dono
-- tem controle total dentro da própria org (escolhe a própria função,
-- igual todo admin já podia) e é protegido: nenhum outro admin da mesma
-- empresa consegue excluí-lo nem alterar as permissões dele — só o
-- super_admin (o Samuel, dono da plataforma) pode, se precisar intervir.
-- Dono não tem NENHUM acesso extra fora da própria empresa — só quem
-- cadastra empresa nova é o super_admin, isso não muda.

alter table public.usuarios add column dono boolean not null default false;

create unique index usuarios_um_dono_por_org_idx
  on public.usuarios (org_id)
  where dono = true;

-- Você é dono da sua própria empresa também, além de dono da plataforma.
update public.usuarios set dono = true where super_admin = true;
...
begin
  ...
  if dono_do_alvo and not super_admin_do_chamador and usuario_id_alvo <> auth.uid() then
    raise exception 'Você não pode alterar as permissões do dono da empresa';
  end if;
...
set search_path = public
as $$
  select u.id, u.nome, au.email, u.created_at, u.papel, u.funcao, u.paginas_permitidas, u.foto_url, u.dono
