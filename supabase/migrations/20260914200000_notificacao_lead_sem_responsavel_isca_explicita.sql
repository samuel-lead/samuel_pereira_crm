-- Ajuste do Samuel: "CRM" sozinho no destaque confundia com o nome do
-- próprio sistema (a isca se chama "CRM"). Deixa explícito "Isca <nome>"
-- quando o lead veio de uma isca — origens que não são isca continuam
-- aparecendo como estão (ex.: "Instagram", "Indicação Closer").
...
begin
  ...
  v_titulo := 'Novo lead: ' || coalesce(
    case
      when new.origem like 'Isca: %' then 'Isca ' || substring(new.origem from 7)
      else new.origem
    end,
    'sem origem'
  );
