-- excluir_usuario travava com "violates foreign key constraint
-- nivel_historico_usuario_id_fkey" porque a função transferia leads,
-- interações, reuniões e comandos pro novo responsável, mas esquecia do
-- histórico de mudança de nível (nivel_historico.usuario_id) — que também
-- aponta pro usuário sendo excluído. Agora esse histórico entra na mesma
-- verificação e no mesmo UPDATE de transferência dos outros quatro.
