...
// Conteúdo da página de um lead — usado tanto na página cheia
// (app/(app)/leads/[id]/page.tsx, pra link direto/compartilhado) quanto no
// pop-up que abre por cima do Kanban (app/(app)/leads/@modal/(.)[id]/page.tsx),
// pra abrir/editar um lead sem sair da aba (Pré-vendas/Vendas/Base) em que a
// pessoa estava.
export async function LeadDetalhe({
  id,
  marcarReuniao,
  ...
  const conteudo = (
    <main
      className={
        variante === "pagina"
          ? "grid gap-6 px-6 py-6 lg:grid-cols-[minmax(0,1fr)_360px]"
          : "grid gap-5 px-5 py-5 lg:grid-cols-[minmax(0,1fr)_320px]"
      }
    >
      <div className="flex flex-col gap-4">
        ...
        <EditarLeadForm
          ...
          reuniaoAnteriorSumiuPredefinido={
            reuniaoAnteriorSumiu === "sim" || reuniaoAnteriorSumiu === "nao"
              ? reuniaoAnteriorSumiu
              : undefined
          }
    ...
    return (
      <>
        <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-xl border-b border-neutral-200 bg-white px-5 py-4 pr-14">
          <h1 className="truncate text-lg font-bold text-neutral-900">{leadTipado.nome}</h1>
        </div>
        {conteudo}
      </>
    );
