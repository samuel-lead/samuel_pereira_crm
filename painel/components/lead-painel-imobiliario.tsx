...
const ABAS = ["Info", "Atividades", "Proposta", "Linha do tempo"] as const;
type Aba = (typeof ABAS)[number];

// Cópia do painel lateral com abas do CRM 100Bug (pedido do Samuel), só
// pro público imobiliário — components/lead-modal-conteudo.tsx (mentoria)
// continua igual, sem nenhuma mudança. Usa exatamente os mesmos
// subcomponentes/ações, só reorganizados em abas em vez de uma coluna
// única rolável.
export function LeadPainelImobiliario({
  dados,
  marcarReuniao,
  ...
  const [aba, setAba] = useState<Aba>(abrirProposta ? "Proposta" : "Info");
  const propostaRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (abrirProposta) setAba("Proposta");
  }, [abrirProposta]);
  ...
  const totalLinhaDoTempo = interacoes.length + reunioes.length + nivelHistorico.length;

  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-neutral-200 bg-white px-3">
        {ABAS.map((nomeAba) => (
          <button
            key={nomeAba}
            type="button"
          ...
          >
            {nomeAba}
            {nomeAba === "Linha do tempo" && totalLinhaDoTempo > 0 && (
              <span className="ml-1.5 text-xs text-neutral-400">{totalLinhaDoTempo}</span>
            )}
          </button>
        ))}
      </div>
        ...
        {aba === "Info" && (
          <div className="flex flex-col gap-4">
            ...
            <EditarLeadForm
              ...
              aoConfirmarTeveProposta={() => setAba("Proposta")}
            />

            {podeEditar && lead.arquivado_em && <ReativarLeadExcluidoButton leadId={lead.id} />}
          ...
          <div className="flex flex-col gap-4">
            ...
            {!podeEditar && (
              <p className="rounded-md border border-dashed border-neutral-300 px-3 py-6 text-center text-xs text-neutral-400">
                Só o responsável (ou um admin) registra atividade nesse lead.
              </p>
            )}
          </div>
        )}
        ...
        {aba === "Proposta" && (
          <div ref={propostaRef}>
            {lead.status === "vendido" ? (
              ...
              <p className="rounded-md border border-dashed border-neutral-300 px-3 py-6 text-center text-xs text-neutral-400">
                Proposta só fica disponível a partir de {Reuniao(publicoOrg).toLowerCase()} marcada.
              </p>
            )}
          </div>
        )}
                  ...
                  <li key={`reuniao-${reuniao.id}`} className="border-l-2 border-amber-300 pl-3">
                    ...
                    {reuniao.closer_id && (
                      <p className="text-xs text-neutral-500">
                        {Sdr(publicoOrg)}: {usuarios.find((u) => u.id === reuniao.closer_id)?.nome ?? "—"}
                      </p>
                    )}
                    {reuniao.resultado && (
